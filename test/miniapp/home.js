...
window.__TEST_DATA__ = {
  ...
  "cases": [
    ...
    },
    {
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "MISSION DONE 버튼 탭 → 인증 제출 화면 이동 (All Done 표시)",
      "precondition": "모든 활성 참여 오늘 인증 완료",
      "action": "MISSION DONE! 버튼 탭",
      "expected": "인증 제출 화면으로 이동, '🎉 All Done!' + 'You've completed all missions for today. Come back tomorrow!' 표시 (추가 인증 불가)",
      "dbCheck": "변경 없음"
    },
    {
      "id": "HM-N-012",
    ...
    },
    {
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "JWT 만료 시 자동 재발급 시도",
      "precondition": "JWT 24시간 만료, Telegram initData 유효 (앱 재진입 상태)",
      "action": "API 호출 → 401 응답",
      "expected": "FE가 자동으로 initData로 JWT 재발급 시도 → 성공 시 원래 요청 재시도, 실패 시 앱 재시작 유도",
      "dbCheck": "변경 없음"
    },
    {
      "id": "HM-E-003",
    ...
    },
    {
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "Badges 표시 (🏆 X Badges)",
      "precondition": "USER_STATS(totalChallengesCompleted=5)",
      "action": "홈 화면 진입 → 히어로 섹션 통계 박스 확인",
      "expected": "통계 3개 박스 중 '🏆 5 Badges' 정확히 표시",
      "dbCheck": "USER_STATS(totalChallengesCompleted=5)"
    },
    {
      "id": "HM-N-020",
      "type": "normal",
      "regression": false,
      "regressionNote": "",
      "scenario": "신규 가입자 자동 챌린지 목록 이동 (isNewUser: true)",
      "precondition": "최초 가입 사용자, 딥링크 접두사 없이 진입 (초대코드/acquisitionSource 유무 무관)",
      "action": "Telegram Mini App 최초 실행 → 인증 완료 (isNewUser: true 응답)",
      "expected": "홈을 건너뛰고 챌린지 목록(/challenges)으로 자동 이동, REGULAR 탭 기본 선택, BackButton으로 홈 복귀 가능",
      "dbCheck": "변경 없음"
    },
    {
      "id": "HM-N-021",
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "Add to Home Screen 배너 표시 (조건 충족)",
      "precondition": "모바일(iOS/Android) + Bot API 8.0+ + 홈화면 미추가 + home_screen_dismissed 플래그 없음 + visit_count >= 2",
      "action": "홈 화면 진입",
      "expected": "공지 배너 아래, 레벨 바 위에 'Add to Home Screen' 배너 카드 표시. 📱 아이콘 + 'Add to Home Screen' 메인 텍스트 + 'Launch with one tap' 서브 텍스트 + X 닫기 버튼. fade-in 애니메이션으로 등장.",
      "dbCheck": "변경 없음"
    },
    {
      "id": "HM-N-022",
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "Add to Home Screen 배너 탭 → 홈화면 추가 + 영구 숨김",
      "precondition": "Add to Home Screen 배너 표시 상태",
      "action": "배너 카드 영역 탭",
      "expected": "Telegram 네이티브 홈화면 추가 다이얼로그 표시 (addToHomeScreen() 호출). 이후 배너 즉시 사라짐. 재진입 시에도 배너 미표시.",
      "dbCheck": "변경 없음 (CloudStorage home_screen_dismissed='true' 저장됨)"
    },
    {
      "id": "HM-N-023",
      "type": "normal",
      "regression": false,
      "regressionNote": "",
      "scenario": "Add to Home Screen 배너 X 닫기 → 영구 숨김 (addToHomeScreen 호출 없음)",
      "precondition": "Add to Home Screen 배너 표시 상태",
      "action": "배너 우측 X 닫기 버튼 탭",
      "expected": "배너 즉시 사라짐. addToHomeScreen() 호출 없음 (네이티브 다이얼로그 미표시). 재진입 시에도 배너 미표시.",
      "dbCheck": "변경 없음 (CloudStorage home_screen_dismissed='true' 저장됨)"
    },
    {
      "id": "HM-N-024",
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "Add to Home Screen 배너 미표시 (Desktop/Web/구형 앱)",
      "precondition": "Desktop 또는 Web 환경 또는 Bot API 8.0 미만 또는 홈화면 이미 추가 또는 visit_count < 2",
      "action": "홈 화면 진입",
      "expected": "Add to Home Screen 배너 미표시. 기존 레이아웃 그대로 (공지 배너 → 레벨 바 직접 연결).",
      "dbCheck": "변경 없음"
    },
    {
      "id": "HM-N-025",
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "Add to Home Screen 배너 조건 확인 중 에러 시 안전 처리",
      "precondition": "Telegram API (checkHomeScreenStatus, CloudStorage) 호출 시 에러 발생 환경",
      "action": "홈 화면 진입",
      "expected": "배너 미표시 (방어 원칙: 기본값=숨김). 홈 화면 나머지 컴포넌트 정상 렌더링. 콘솔 에러 없음.",
      "dbCheck": "변경 없음"
    }
  ]
};
