...
window.__TEST_DATA__ = {
  ...
  "cases": [
    ...
    },
    {
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "챌린지 생성 시 유형 선택 (REGULAR/EVENT 드롭다운)",
      "precondition": "챌린지 생성 폼 진입",
      "action": "유형 드롭다운에서 REGULAR / EVENT 선택 확인",
      "expected": "REGULAR/EVENT 선택 가능, 기본값 REGULAR, 선택에 따라 폼 표시 분기",
      "dbCheck": "변경 없음"
    },
    {
      "id": "CM-N-018",
      ...
      "regression": false,
      "regressionNote": "",
      "scenario": "EVENT 챌린지 생성 — 카테고리 자유 선택",
      "precondition": "active 카테고리 2개 이상 존재",
      "action": "유형 EVENT 선택 → 카테고리 드롭다운 확인 → 아무 카테고리 선택 → 나머지 필수 필드 입력 → 저장",
      "expected": "성공 토스트, 목록에 새 EVENT 챌린지 표시 (선택한 카테고리 표시)",
      "dbCheck": "CHALLENGE(challenge_type=EVENT, category_id=선택한 카테고리), ADMIN_ACTIVITY_LOG(action=CREATE)"
    },
    {
      "id": "CM-N-019",
      "type": "normal",
      "regression": false,
      "regressionNote": "",
      "scenario": "고정 리워드 설정 (fixedRewardTon)",
      "precondition": "챌린지 생성 폼 진입",
      "action": "고정 리워드 체크박스 체크 → 금액 입력 (0.3 TON) → 저장",
      "expected": "rewardRate 입력 필드 비활성화, 성공 토스트, 목록에 챌린지 표시",
      "dbCheck": "CHALLENGE(fixed_reward_ton=0.3000, reward_rate=0)"
    }
  ]
};
