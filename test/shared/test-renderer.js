...
(function () {
  ...
  if (currentFile) {
    ...
    // 카테고리만 선택된 상태면 첫 번째 feature 자동 선택
    if (currentCategory && manifest[currentCategory] && manifest[currentCategory].items.length > 0) {
      currentFile = manifest[currentCategory].items[0].file;
      updateHash();
      renderNav();
      loadTestData(currentFile);
    } else {
